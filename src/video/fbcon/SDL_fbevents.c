...
#define SDL_INPUT_CHUMBY 1
#if SDL_INPUT_CHUMBY
#define CHUMBY_TS_DEV "/dev/input/by-id/soc-noserial-event-ts"
#define CHUMBY_KBD_DEV "/dev/input/by-id/soc-noserial-event-kbd"
...
#if SDL_INPUT_CHUMBY
		if((keyboard_fd = open(CHUMBY_KBD_DEV, O_RDONLY)) >= 0)
			return(keyboard_fd);
#endif /* SDL_INPUT_CHUMBY */
...
#if SDL_INPUT_CHUMBY
	if((mouse_fd = open(CHUMBY_TS_DEV, O_RDONLY)) > 0) {
		mouse_drv = MOUSE_CHUMBY;
		return mouse_fd;
	}
#endif /* SDL_INPUT_CHUMBY */
...
#if SDL_INPUT_CHUMBY
static void handle_chumby_mouse(_THIS)
{
	struct input_event event;
	while(1) {
		int x, y, click;
		int got_x=0, got_y=0;
		if(sizeof(event) != read(mouse_fd, &event, sizeof(event)))
			return;
		...
		if(event.type == EV_ABS) {
			...
			if(got_x && got_y)
				FB_vgamousecallback(click<<2, 0, x, y);
			break;
		}
	}
}
...
static void handle_keyboard(_THIS)
{
	...
	int scancode;
	SDL_keysym keysym;
#ifdef SDL_INPUT_CHUMBY
	struct input_event event;

	if(sizeof(event) != read(keyboard_fd, &event, sizeof(event)))
		return;
	...
	/* Determine whether this is a press- or release-event */
	pressed = SDL_RELEASED;
	if(event.value)
		pressed = SDL_PRESSED;

	/* Translate the key to an SDL value and post it to the key queue */
	TranslateKey(event.code, &keysym);
...
void FB_InitOSKeymap(_THIS)
{
...
#if SDL_INPUT_CHUMBY
	keymap[KEY_RESERVED] = SDLK_UNKNOWN;
	...
	/*
	keymap[SDLK_ ]            = KEY_COMMA;
	keymap[SDLK_ ]            = KEY_DOT;
	keymap[SDLK_ ]            = KEY_SLASH;
	keymap[SDLK_ ]            = KEY_RIGHTSHIFT;
	keymap[SDLK_ ]            = KEY_KPASTERISK;
	keymap[SDLK_ ]            = KEY_LEFTALT;
	keymap[SDLK_ ]            = KEY_SPACE;
	keymap[SDLK_ ]            = KEY_CAPSLOCK;
	keymap[SDLK_ ]            = KEY_F1;
	keymap[SDLK_ ]            = KEY_F2;
	keymap[SDLK_ ]            = KEY_F3;
	keymap[SDLK_ ]            = KEY_F4;
	...
	keymap[SDLK_ ]            = KEY_F8;
	keymap[SDLK_ ]            = KEY_F9;
	keymap[SDLK_ ]            = KEY_F10;
	keymap[SDLK_ ]            = KEY_NUMLOCK;
	keymap[SDLK_ ]            = KEY_SCROLLLOCK;
	keymap[SDLK_ ]            = KEY_KP7;
	keymap[SDLK_ ]            = KEY_KP8;
	keymap[SDLK_ ]            = KEY_KP9;
	keymap[SDLK_ ]            = KEY_KPMINUS;
	keymap[SDLK_ ]            = KEY_KP4;
	*/
